...
```
Lighthouse accessibility score gate (> 90) is enforced via `--assert` flag or `lhci` configuration, not in the budget file.
### CI Job: `axe-scan`
```yaml
axe-scan:
...
Automated scanning covers ~30-40% of accessibility issues. These require manual verification:
...
- **Focus management** -- Focus moves logically after modal dialogs, route changes, dynamic content updates
