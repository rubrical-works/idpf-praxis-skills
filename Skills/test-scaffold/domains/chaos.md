...
**Principle:** Proactively test system resilience by introducing controlled failures to discover weaknesses before production incidents.
...
Toxiproxy-based latency injection experiment with hypothesis and abort conditions.
...
Dependency failure experiment simulating complete upstream outage.
...
Toxiproxy proxy configuration for local and CI chaos testing.
...
Chaos engineering requires human judgment for many resilience aspects. The following cannot be fully automated:

- **GameDay facilitation** -- Coordinated multi-team chaos exercises require planning, communication, and real-time decision making
- **Blast radius assessment** -- Evaluating whether experiment scope is safe for the target environment
- **Hypothesis validation** -- Interpreting whether observed behavior matches the hypothesis or reveals a new finding
- **Rollback verification** -- Confirming full system recovery after experiment completion
- **Runbook accuracy** -- Verifying that documented procedures match actual recovery steps
- **Approval gates** -- Production experiments require sign-off from service owners, SRE leads, or engineering managers
- **Incident response testing** -- Validating that on-call procedures and escalation paths work during simulated failures
- **Cross-service impact** -- Observing whether fault injection in one service causes unexpected cascading failures

**Progressive Complexity (from framework):**
