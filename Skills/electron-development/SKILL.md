...
**Always commit after each working change.** Batching many file changes without committing is how work is lost.
- Bad: change package.json, forge.config.ts, HTML, TypeScript, icons, tests — then a failure loses all of it.
- Good: change package.json -> commit; change forge.config.ts -> commit; and so on.
...
## Resources
Detailed guides are in `resources/`, linked from the Quick Reference table.
