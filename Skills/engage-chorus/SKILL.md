...
|---|---|---|
| **Primary** | Node.js 18+ | Validates stakeholder briefs, mediator output, proposal template via `ajv` (when present). |
...
2. **Node unavailable** → surface Pattern 4 diagnostic: *"engage-chorus refracts one multi-party negotiation into N stakeholder-advocate paths (one per party), enforces steel-man contract on each, runs named mediator subagent producing negotiated-outcome landscape (ZOPA, trade frontier, settlement zones, unresolved conflicts, concession sequencing, coalition map), writes proposal with concession-sequencing recommendation. With Node, stakeholder briefs and mediator output validated against bundled JSON Schemas using `ajv`. Without Node, Claude performs same structural checks inline. Cost: validation approximate rather than ajv-grade; no other feature degraded. Or install Node.js 18+ from https://nodejs.org/."* Then execute Fallback.
...
Drift bounded by contract tests under `tests/skills/engage-chorus/`.
...
Path count (`N`) determined by situation, NOT by `--paths` flag. Typical: 2–6 stakeholders. Two paths with same role is a contract violation.
...
1. **The negotiation.** What's being negotiated — restated in 1–2 sentences.
...
- **Default (`--stakeholders` not supplied):** Agent infers from situation framing. Typical N: 2–6. Names each with kebab-case role identifier (`"the-regulator"`, `"the-vendor"`, `"the-end-user"`, `"the-product-team"`) and one-sentence characterization.
...
Spawn all N **at the same time**. Each produces stakeholder brief conforming to `resources/stakeholder-brief-schema.json` (AC3). Six mandatory fields:
...
- **Strong:** Interests as real motivations; non-trivial BATNA; concrete tradeable concessions; specific coalition signals.
...
3. **Stakeholders** — One section per advocate with full brief.
4. **Mediator Landscape** — ZOPA, trade frontier, settlement zones, unresolved conflicts, concession sequencing, coalition map.
5. **Recommended Concession Sequence** — Ordered moves to test deal space.
