...
## YAGNI Principle
**Y**ou **A**in't **G**onna **N**eed **I**t -- Don't implement features until you have a test requiring them.
...
```
**YAGNI Checklist:** Before adding code, ask:
...
**Make it work, then make it right, then make it fast.** In GREEN phase, focus only on "make it work."
**When to optimize:** After tests pass (REFACTOR phase), when performance tests fail, when profiling shows bottleneck. Never optimize based on assumptions.
