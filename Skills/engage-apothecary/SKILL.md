...
1. Load-bearing refusal gate at preflight (deterministic).
...
Applies **No-Runtime Fallback Pattern**. Two paths chosen by preflight:

| Path | Requires | What it does |
|---|---|---|
| **Primary** | Node.js 18+ | Validates citation/red-flag/Bayesian/differential-brief schemas via `ajv` (when present). |
| **Fallback** | None | Claude reads schemas under `resources/` and performs structural checks inline. Higher token cost; no `ajv`-grade validation. |
...
**WebFetch / WebSearch** required by default for evidence-tier citations. With `--no-web`, each brief records degradation; citation tier capped at "expert opinion."

### Preflight (before Step 0)

Two checks in sequence:

**(1) Point-of-care detection** (deterministic, non-bypassable):
...
The refusal gate runs deterministically on both paths.
...
- **Any individual patient.** "My patient has..." — refused.
- **Dosing.** "What dose of X for a Y-kg adult?" — refused.
- **Prescribing.** "Should I prescribe X or Y?" — refused.
- **Acute symptoms.** "Chest pain right now" — refused with emergency-care redirect.
- **Patient-identifying details.** Names, DOBs, MRNs, specific dates/locations — refused.
...
| `--no-web` | Skip web research; citation tier capped at expert-opinion | *(web on)* |
...
PRIMARY AGENT
     ...
     │       └── ANY refusal pattern matches → REFUSE + HALT
...
**First contract** of this skill. **Deterministic** (regex pattern matching), **non-bypassable**.
...
4. **On no match**, proceed to Preflight B.

Matches **structural triggers** — phrasing indicating point-of-care. Not a content classifier. False positives acceptable; false negatives are not.

### Why deterministic?

LLM-judgment-based refusal is fragile — adversarial rephrasings, edge cases, model-update drift produce inconsistent results. Named categories are well-bounded enough for regex matching. Accepts higher false-positive rates for zero false negatives.
...
**Cannot be suppressed by any flag.** `--no-proposal` skips proposal but disclaimer appears in stdout. `--no-web` does not affect. No suppression flag exists and none can be added.
...
- Records evidence-tier preferences (default: systematic-review and RCT preferred; case-series and expert-opinion flagged) and recency window (default 10 years; `--recency-years N` overrides).

If user input cannot be reframed without losing substance, prompt user to reformulate. Skill does not silently strip identifying information.

## Step 2 — Generate Differentials

Identify N candidate diagnoses (typically 3-5, default 3) using:
1. **Mnemonic-driven enumeration** (e.g., VITAMIN-CDE — Vascular/Infectious/Toxic/Autoimmune/Metabolic/Iatrogenic/Neoplastic/Congenital/Degenerative/Endocrine).
2. **Epidemiologic priors** for the presentation in framed demographic.
3. **Red-flag triage** — explicitly flag must-not-miss for Step 4.
...
Spawn all N at the same time. Each produces a structured brief conforming to `resources/differential-brief-schema.json`:
- **diagnosis** — name + ICD code if well-known.
- **mechanism** — pathophysiologic reasoning, ≤150 words.
...
- **contraindicationReasoning** — what would argue against this diagnosis.

### Web research

Each subagent performs live WebFetch/WebSearch for evidence-tier citations within recency window. Outside-window sources flagged but not rejected. With `--no-web`, brief records `webResearch: false` and citation tier capped at expert-opinion.

### Subagent fabrication check

Citations checked against `attemptedCalls` record (mirrors `/engage-prism`). Citations without recorded WebFetch/WebSearch flagged as `webResearchSuspect: true`.

## Step 4 — Red-Flag Advocate (Mandatory)

**Dedicated single-role subagent** spawned every invocation regardless of N. Brief:
...
Conforms to `resources/red-flag-advocate-output-schema.json`:
- **mustNotMissDiagnosis** — the must-not-miss differential.
- **sharingFeaturesRationale** — what makes this plausibly share the presentation.
...
**Always present.** No skip flag. If no plausible must-not-miss exists (rare), advocate must explicitly declare `mustNotMissDiagnosis: "none-identified"` with explanation — silent omission forbidden.

### Why always-mandatory and not optional?

Clinical reasoning omitting red-flag check teaches premature closure. The cognitive skill of *always asking "what's the must-not-miss?"* is load-bearing pedagogy. Making the role optional would let outputs model the failure mode the skill teaches learners to avoid.

## Step 5 — Bayesian Pre/Post-Test Synthesis (Mandatory)

**Required synthesis.** Runs every invocation. Produces probability landscape turning "here are differentials" into "here is how evidence updates the landscape."

### Synthesis procedure

1. **Pre-test probability per differential** — based on framed scenario's demographics + presentation. When literature provides published pretest probabilities, cite with tier. Otherwise declare `assumption-based: true` and state assumption explicitly.
2. **Highest-impact test/finding** — identify single test or finding that would most update the landscape. Use likelihood ratios where well-published (LR+ ≥ 10 for "strong rule-in"; LR− ≤ 0.1 for "strong rule-out"); qualitative ranking otherwise.
3. **Post-test probability landscape** — for each differential, name post-test probability after highest-impact test result (positive AND negative cases). When LRs not well-published, name qualitative direction.
4. **Educational learning objective** — name the specific Bayesian intuition or test characteristic this case teaches. Examples: "high pre-test probability + moderate-LR test still leaves substantial residual probability"; "low pre-test probability + high LR+ raises post-test probability but not to action threshold"; "a sensitive test with poor specificity is for ruling out, not ruling in."
...
- **highestImpactTest** — object with `test`, `targetDifferentials`, `likelihoodRatios` (when published) or `qualitativeImpact` (when not).
...
Bayesian updating is what the skill is *teaching*. Without synthesis, the output is a list of differentials — useful as reference, useless as pedagogy. Mandatory synthesis turns the skill into a reasoning trainer.

## Step 6 — Stamp Educational-Only Disclaimer

Disclaimer read from `resources/disclaimer.txt` and stamped verbatim on every output: stdout, proposal, every example. Disclaimer is closing element of proposal and appears in stdout summary.
...
2. **Metadata** — Date, skill, differential count, recency window, web-research flag.
...
| Node missing | Surface Pattern 4; switch to Fallback. No halt. Refusal gate still runs. |
...
| Bayesian synthesis missing `educationalLearningObjective` | Reject + re-dispatch. Learning objective is pedagogical core. |
| Subagent returns citations without recording WebFetch/WebSearch | Flag `webResearchSuspect: true`; brief retained but flagged. |
| Input contains both refusal trigger AND educational framing | Refusal gate wins. User can reframe. |

## Important Constraints

- **Refusal gate is load-bearing.** No flag suppresses. Named categories deterministically detected by regex.
- **Educational-only disclaimer is non-suppressible.** Verbatim on every output. No suppression flag exists.
- **Red-flag advocate is mandatory.** One per invocation regardless of N.
- **Bayesian pre/post-test synthesis is mandatory.** Every invocation produces landscape + learning objective.
- **Evidence tiers required on every citation.** Unknown tiers fail validation. Fabricated quantitative LRs forbidden; qualitative ranking when not well-published.
- **Paradigm catalog is palette, not routing.** `match-signals.js` never consumed.
...
| File | Purpose |
|---|---|
| `refusal-gate-patterns.json` | Deterministic refusal triggers (5 categories) + redirect messages |
| `disclaimer.txt` | Verbatim educational-only disclaimer |
...
| `red-flag-advocate-output-schema.json` | Red-flag-advocate output schema |
...
| Quality gates | Citation validation + recency | Refusal gate (load-bearing) + evidence-tier + red-flag advocate (mandatory) + Bayesian synthesis (mandatory) + non-suppressible disclaimer |
| Web research | Mandatory by default (`--no-web` overrides) | Mandatory by default (`--no-web` caps tier at expert-opinion) |
| Refusal contract | None | Load-bearing refusal gate at preflight |
