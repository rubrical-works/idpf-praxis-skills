...
Applies **No-Runtime Fallback Pattern** (see `SKILL-DEVELOPMENT-GUIDE.md` → No-Runtime Fallback Pattern, and `Construction/Design-Decisions/2026-04-26-no-runtime-fallback-pattern.md`). Mirrors `/debate-prism` post-#252 preflight contract. Does not consume `match-signals.js` or any shared script, so only Node use is `ajv`-based JSON Schema validation. Fallback skips schema validation; rest runs unchanged.
...
Operators with hard schema-validation requirement should install Node.js 18+ and `ajv`.
## When to use
...
| `--round-two` | Force second round (re-dispatch for-advocate against the against-advocate's strongest citation) regardless of judge confidence | *(off — judge triggers)* |
...
Spawn both **at the same time** using the Agent tool. Each receives brief from `resources/for-brief-template.json` or `resources/against-brief-template.json`, with `{jurisdiction}` propagated as immutable input.
### For-advocate brief
