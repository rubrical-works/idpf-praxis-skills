...
This document defines the pattern scanning rules and category mapping logic for the `code-path-discovery` skill. The skill uses regex-based heuristics and structural analysis (not AST) to identify behavioral patterns in TypeScript/JavaScript source files.
...
Scan for patterns that define HTTP routes or entry points:
...
Scan for primary handler logic — the main flow of a route or function:
...
Scan for branching logic that represents alternative flows:
...
Scan for error handling constructs:
...
Scan for early-return guards and boundary validation:
...
Scan for compound conditions that combine multiple checks:
...
Scan for input validation and authorization:
