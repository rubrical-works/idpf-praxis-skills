...
|---|---|---|
| **Primary** | Node.js 18+ | Validates narrative artefact schemas, synthesis-output, critique-output via `ajv` (when present). |
...
Drift bounded by contract tests under `tests/skills/engage-codex/`.
...
Primary agent validates each returned artefact before synthesis. Failure modes:
...
No operational-graft escape hatch. Forge has one (heuristic-axis-specific element transplant); codex does not.
...
2. **Subagent walks every path's artefact** beat-by-beat (or scene-by-scene), flagging findings per category.
...
"Explicit clean OR findings" mirrors engage-chorus mediator-output discipline: silent omission is dishonest because user can't distinguish "critique found nothing" from "critique skipped this category".
...
- **Synthesis is comparison, not merger.** No operational-graft escape hatch.
...
- **No heuristic audit step.** Narrative critique IS the audit.
...
| `artefact-schemas/beat-sheet.schema.json` | Paradigm-specific beat sheet schema |
| `artefact-schemas/scene-outline.schema.json` | Scene-by-scene outline schema |
...
See `Skills/MAINTENANCE.md` → Artefact-First Refraction Patterns.

**End of engage-codex SKILL.md**
