...
Pattern slot: **artefact-producing refraction + constructive falsification attack + Bayesian synthesis**. Popperian — conjecture, attempt refutation, measure information gain.
...
|---|---|---|
| **Primary (default)** | Node.js 18+ on `PATH` | Validates hypothesis briefs, experimental-design artefacts, attack outputs, Bayesian-synthesis outputs, citations against bundled JSON Schemas via `ajv` (when present). |
...
Both paths produce the same artefact + attack + synthesis shapes. Path selected once at preflight.

**Web research is optional.** Pass `--offline` to declare offline reasoning explicitly.

**No `sharedScripts:`.** Does **not** consume `match-signals.js` or the engage-* paradigm-routing matcher. Paradigm catalog is an **optional inspiration palette**, not a routing substrate.
...
2. **Experimental-design artefact validation.** Read artefact schema under `resources/artefact-schemas/` matching brief's declared `artefactType` (`pico` | `peco` | `generic-experimental-setup`). Reject pure-prose paths without a structured payload.
...
Without `--offline`, web research is opportunistic — citations preferred when available, absence does not block hypothesis generation.
## When to use
...
Primary agent identifies N competing hypotheses (default 3; 2–4 valid). Each must be **falsifiable in principle** — some observation must disprove it. Unfalsifiable hypothesis ("complexity emerges from holism") rejected and re-formulated.

Each hypothesis here is one paragraph: claim + proposed mechanism + sketch of what would falsify. Full brief (declared falsification condition + experimental-design artefact) produced by subagent in Step 2.
### Hypothesis-diversity requirement
N hypotheses must propose **genuinely different mechanisms**, not one mechanism with different parameters. Primary agent verifies before dispatch — if two collapse to "same explanation, different magnitudes," merge them and generate a fresh distinct hypothesis.
## Step 2 — Dispatch N Hypothesis Subagents in Parallel
Spawn all N **at once** via the Agent tool. Each gets a slot-filled brief and produces:
1. **Hypothesis brief** conforming to `resources/hypothesis-brief-schema.json`, with mandatory fields:
   - `id` (kebab-case)
   - `claim` (precise statement of the hypothesis)
   - `mechanism` (proposed causal mechanism)
   - `predictions[]` (observable consequences if true)
   - `falsificationCondition` (specific observation that would disprove it — **mandatory**, **AC3 gate**)
   - `priorEvidenceBase[]` (citations to existing evidence, with `evidenceTier`)
   - `assumptions[]` (load-bearing assumptions whose violation would invalidate the hypothesis)
...
Each brief MUST declare a non-empty `falsificationCondition`. Briefs failing this gate are rejected:
...
Primary agent validates each returned artefact against its declared schema. Pure-prose returns (no structured artefact, or artefact missing required schema fields) fail with same re-dispatch / second-failure-tagging pattern.
## Step 3 — Falsification Attack Pass (Mandatory)
One attacker subagent per hypothesis. Job is **constructive, not rhetorical** — it does NOT break the hypothesis; it designs the **cheapest experiment discriminating the assigned hypothesis from its siblings**.

Scientific analog of `/spar-exocortex`'s failing-input attack: a concrete experiment distinguishing the assigned hypothesis from its siblings.
### Attack brief (slot-filled)
Read `resources/attack-brief-template.json`. For each attacker, fill:
- `assignedHypothesis` — hypothesis this attacker paired with.
- `siblingHypotheses` — other N-1 hypotheses.
- `evidenceTierPreferences` — inherited from Step 0.
...
Adversarial attack ("why H1 is wrong") produces one-sided dismissal; constructive attack ("cheapest test discriminating H1 from H2/H3") produces actionable design. Attacker is on the hypothesis-set's side, not against any individual hypothesis.
## Step 4 — Bayesian Prior-Update Synthesis (Mandatory)
Synthesis replaces winner-picking with a **prior-update landscape**: given N hypotheses, their priors, and the N attacker-proposed crucial experiments, what is the posterior landscape and which experiment resolves most uncertainty per dollar?
### Prior
...
### Posterior landscape
For each proposed crucial experiment, compute (or qualitatively rank) posterior probability per hypothesis if its discriminating prediction holds. Full Bayesian computation optional — qualitative ranking acceptable (`up | unchanged | down` per hypothesis per experiment).
### Highest-information-gain-per-cost
Across all N experiments, identify the single one resolving most uncertainty per cost unit. This is the **crucible test** — run it first.

Information-gain ranking is qualitative when tractable, qualitative-with-rationale when not. Cost uses the attack-pass tiers (`trivial | cheap | moderate | expensive | prohibitive`).
### Research roadmap
Ordered sequence resolving most uncertainty soonest. First item is the crucible test; subsequent items become valuable conditional on its outcome.
### Learning objective
...
| Hypothesis brief missing `falsificationCondition` | Reject + re-dispatch; second failure tags `unfalsifiable` and excludes from attack + synthesis (AC3). |
...
- **Falsifiability is mandatory.** Each hypothesis declares specific, measurable observation that would disprove it. Unfalsifiable hypotheses fail validation.
...
- **Paradigm catalog is palette, not routing.** Selective opens fine; full-catalog routing through `match-signals.js` is a contract violation.
- **No `sharedScripts:`.** Does not consume `match-signals.js` or any shared script.
...
| `attack-output-schema.json` | Falsification-attack output shape (discriminating prediction + siblings-ruled-out + setup + info-gain + cost) |
...
- **Falsifiability gate** — each hypothesis declares the observation that would disprove it; unfalsifiable rejected.
- **Bayesian prior-update synthesis** — posterior-update landscape + crucible-experiment identification + research roadmap, not winner-picking.
- **Evidence-tier-weighted citations** — explicit tier (systematic-review > RCT > cohort > observational > expert > anecdote).
- **`--offline` first-class** — pure-reasoning mode without `attemptedCalls` violations.
